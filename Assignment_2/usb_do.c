...
#define HDD_1_VID  0x0bc2
#define HDD_1_PID  0x231a

#define HDD_2_VID  0x0bc2
#define HDD_2_PID  0xac19

#define ONE_PLUS_VID  0x2a70
#define ONE_PLUS_PID  0xf003
...
static int usbdev_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	{
		printk(KERN_INFO "Seagate HDD_1 drive detected.\n");
	}
	else if((id->idProduct == HDD_2_PID))
	{
		printk(KERN_INFO "Seagate HDD_2 drive detected.\n");
	}
	else if((id->idProduct == ONE_PLUS_PID))
	{
		printk(KERN_INFO "One Plus Mobile detected.\n");
	}
	else{
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Prathamesh Gujar <h20191400075@hyderabad.bits-pilani.ac.in> ");
MODULE_DESCRIPTION(" USB DEVICE DETECTOR");
